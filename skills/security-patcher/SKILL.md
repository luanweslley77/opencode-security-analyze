...
1. **Pre-Requisites:**
    *   Check for the existence of a security report in the `.opencode_security/` directory.
    *   If a security report does not exist, kick off a `security_analyze` tool call to build the required security context before proceeding.
...
5. **Verify the Vulnerability Exists (Before Patching):**
    *   If a PoC doesn't exist, use the `poc` skill to generate one.
...
7. **Verify the Vulnerability is Fixed (After Patching):**
    *   If you generated or verified a PoC in step 5, execute the PoC again using the `run_poc` tool **after** applying your patch.
