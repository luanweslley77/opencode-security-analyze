...
1. **Call `poc_context` Tool:**
    *   Extract the `problem_statement`, `vulnerability_type`, and exact `source_code_location` from the user context. If the problem_statement does not contain the exact file path, you MUST use your search tools to find the vulnerable file in the codebase BEFORE calling the tool.
